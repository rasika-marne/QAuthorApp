...
- (void)authenticateUserWithCallbackObject:(id)anObject andSelector:(SEL)selector andExtendedPermissions:(NSString *)extended_permissions andSuperView:(UIView *)super_view {
	...
	webFrame.origin.y = 45.0f;
    fbConnectView=[[UIView alloc]initWithFrame:CGRectMake(0.0f,0.0f,320.0f,45.0f)];
    [fbConnectView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage   imageNamed:@"fbg.png"]]];

    titleLabel= [[UILabel alloc] initWithFrame:CGRectMake(3.0f,15.0f,90.0f,18.0f)];
    titleLabel.backgroundColor = [UIColor clearColor];
    [titleLabel setTextColor:[UIColor whiteColor]];
    titleLabel.font = [UIFont fontWithName:@"Helvetica Bold" size:12.0f];
    titleLabel.text = @"Facebook";
    [fbConnectView addSubview:titleLabel];
        
    UIImage* closeImage = [UIImage imageNamed:@"facebookOverlayClose.png"];
    
   closeButton = [UIButton buttonWithType:UIButtonTypeCustom];
   [closeButton setFrame:CGRectMake(298.0f,13.0f,20.0f,20.0f)];
   ...
   [closeButton addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
	...
	[super_view addSubview:self.webView];
    [super_view addSubview:fbConnectView];
    [fbConnectView addSubview:closeButton];
    activity=[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    activity.frame=CGRectMake(160.0f,198.0f,20.0f,20.0f);
...
-(void)cancel {
    ...
    [fbConnectView removeFromSuperview];
}
